...
describe('Given a UserController class', () => {
  describe('When it is instantiated', () => {
    ...
    test('Then the register method should be used', async () => {
      req.body = { userName: 'Ernestina', passwd: 'tomate' };
      // Act
    ...
    test('Then the login method should be used', async () => {
      req.body = { user: 'testErnestina', passwd: '12345' };
...
describe('Given a user controller', () => {
  const req = {
    body: { user: 'Ernestina', passwd: '12345' },
  ...
  describe('When it is instantiated and login method is called but userIsValid is false', () => {
    test('Then it should throw an error', async () => {
      req.body.userName = 'Ernestina';
      req.body.passwd = '12345';
      const mockRepo: UserRepo = {
        search: jest
          .fn()
          .mockResolvedValueOnce([{ user: 'Ernestina', password: '12345' }]),
      ...
      const mockRepo: UserRepo = {
        search: jest
          ...
          .mockResolvedValueOnce([
            { userName: 'Ernestina', passwd: 't12345', id: '1' },
          ]),
